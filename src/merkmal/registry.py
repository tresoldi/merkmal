...
from merkmal.dataset import FeatureDataset, load_builtin_dataset
...
def create_registry(
    ...
    *,
    ...
    if register_builtin:
        registry.register("descriptive", DescriptiveFeatureSystem(dataset_obj))
        registry.register("broad", BroadFeatureSystem(dataset_obj))
        ...
        registry.register("phoible", PhoibleFeatureSystem())
        registry.register("classfeat", ClassFeatFeatureSystem())
    return registry


def _adapt_grapheme(grapheme: str, transcription: str | None) -> str:
    """Adapt a grapheme from a non-IPA transcription system to canonical form."""
    if transcription is None:
        return grapheme
    if transcription == "upa":
        from merkmal.upa import adapt_segment
        return adapt_segment(grapheme)
    msg = f"Unknown transcription system: {transcription!r}. Available: ['upa']"
...
def get_features(
    ...
    *,
    ...
    When *transcription* is given (e.g. ``"upa"``), the grapheme is
    first adapted to the canonical (IPA) form before feature lookup.
    """
    grapheme = _adapt_grapheme(grapheme, transcription)
...
def segment_distance(
    ...
    *,
    ...
    When *transcription* is given and *a*/*b* are strings, they are
    adapted to canonical IPA and then converted to native representations
    before computing distance.
    """
    sys = get_system(system)
    if transcription is not None and isinstance(a, str) and isinstance(b, str):
        a_ipa = _adapt_grapheme(a, transcription)
        b_ipa = _adapt_grapheme(b, transcription)
        rep_a = sys.grapheme_to_representation(a_ipa)
        rep_b = sys.grapheme_to_representation(b_ipa)
        if rep_a is None or rep_b is None:
            msg = f"Cannot compute distance: no representation for {a_ipa!r} or {b_ipa!r}"
            raise ValueError(msg)
        return sys.segment_distance(rep_a, rep_b, node_weights)
