...
from merkmal.registry import get_system
...
def _compose_expand(
    ...
    *,
    exact: bool,
    compose: bool | list[str],
) -> None:
    """Expand *found* with compositionally generated graphemes.

    Mutates *found* and *seen* in place.
    """
    if compose is True:
        all_mods = available_modifiers()
        modifier_subsets: list[tuple[str, ...]] = [
            (m,) for m in all_mods
        ]
    ...
    for base_grapheme in system_obj.list_graphemes():
        base_features = system_obj.grapheme_to_features(base_grapheme)
        if base_features is None:
            continue
        for mod_tuple in modifier_subsets:
            mod_set = frozenset(mod_tuple)
            candidate_features = base_features | mod_set
            matched = (
                candidate_features == query
                if exact
                else system_obj.partial_match(query, candidate_features)
            )
            if matched:
                composed = compose_grapheme(base_grapheme, mod_set)
...
def features_to_graphemes(
    ...
    *compose* controls compositional expansion for categorical systems:

    - ``False`` (default): only table graphemes are considered.
    - ``True``: each table grapheme is tried with every single modifier
      and every pair of modifiers from the full modifier set.
    - A list of modifier feature names: each table grapheme is tried
      with every non-empty subset of the given modifiers.
    """
    ...
    for grapheme in system_obj.list_graphemes():
        ...
        if matched and grapheme not in seen:
            found.append(grapheme)
            seen.add(grapheme)

    if compose is not False:
        _compose_expand(
            query, system_obj, found, seen,
            exact=exact, compose=compose,
        )
